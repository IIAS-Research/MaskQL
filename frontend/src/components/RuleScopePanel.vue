...
function badgeTone(key: string) {
  return props.badgeToneOf?.(key) ?? props.statusOf(key);
}
function badgeLabel(key: string) {
  return (
    props.badgeLabelOf?.(key) ??
    (props.statusOf(key) === "inherit"
      ? "hérite"
      : props.statusOf(key) === "allow"
      ? "allow"
      : "deny")
  );
}
  ...
  <div class="bg-white rounded-2xl shadow p-3 h-full flex flex-col">
      ...
      >
        <div class="flex items-start justify-between gap-3">
          ...
          <div class="flex items-start gap-2 shrink-0">
            <div class="inline-flex border rounded-lg">
              <button
                :class="segBtn(statusOf(it.key) === 'allow')"
                @click.stop="onAllow(it.key)"
                title="Autoriser"
              >
                <i class="pi pi-check text-xs"></i>
              </button>
              <button
                :class="segBtn(statusOf(it.key) === 'deny')"
                @click.stop="onDeny(it.key)"
                title="Refuser"
              >
                <i class="pi pi-ban text-xs"></i>
              </button>
              <button
                :class="segBtn(statusOf(it.key) === 'inherit')"
                @click.stop="onInherit(it.key)"
                title="Hériter"
              >
        ...
        <div class="mt-2 flex items-center gap-2 flex-wrap">
          <span
            ...
            :class="badgeClass(badgeTone(it.key))"
          >
            {{ badgeLabel(it.key) }}
          </span>
        </div>

        <div v-if="showEffect" class="mt-2">
          ...
          <input
            ...
            @input="
              setEffect?.(it.key, ($event.target as HTMLInputElement).value)
            "
    ...
    <div v-if="showAdd" class="space-y-2 border-t pt-3 mt-auto">
      <input
        :value="addModel"
        @input="
          emit('update:addModel', ($event.target as HTMLInputElement).value)
        "
